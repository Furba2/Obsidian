Each type of data need right container.
...
```
 ...
 Each column in SQL table can hold **one and only one** data type. You declare it in `CREATE TABLE`.
...
|------|----------|---------|
| `char(n)` | | `char(10)` always stores 10 chars |
| `varchar(n)` | Variable length, max n | `varchar(10)` stores only what you insert |
| `text` | Variable length, unlimited (up to ~1GB) |  |
...
> ⚠️ **Gaps happen:** If row deleted or insert aborted, sequence still increase.
...
JSON = **JavaScript Object Notation** — structured format key/value pairs.
...
you need to convert value from one type to another.
